...
// NewDagReader creates a new reader object that reads the data represented by
// the given node, using the passed in DAGService for data retrieval.
func NewDagReaderAlt(ctx context.Context, n ipld.Node, serv ipld.NodeGetter, mechanism string) (DagReader, error) {
	var size uint64
	var mode os.FileMode
	var modTime time.Time
	...
	case *mdag.ProtoNode:
		...
		case unixfs.TMetadata:
			...
			return NewDagReaderAlt(ctx, childpb, serv, mechanism)
		case unixfs.TSymlink:
			return nil, ErrCantReadSymlinks
		default:
			return nil, unixfs.ErrUnrecognizedType
