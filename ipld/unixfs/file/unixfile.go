...
func NewUnixfsFileAlt(ctx context.Context, dserv ipld.DAGService, nd ipld.Node, mechanism string) (files.Node, error) {
	switch dn := nd.(type) {
	case *dag.ProtoNode:
		fsn, err := ft.FSNodeFromBytes(dn.Data())
		if err != nil {
	...
	dr, err := uio.NewDagReaderAlt(ctx, nd, dserv, mechanism)
	if err != nil {
		return nil, err
	}
